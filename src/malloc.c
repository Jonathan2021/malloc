...
allocate(void)
{
    return mmap(NULL, MEM, PROT_READ | PROT_WRITE | PROT_EXEC,
...
static void
add_alloc(struct chunk *c, size_t size)
{
...
static void
wordcpy(void *dst, void *src, size_t len)
{
    memcpy(dst, src, len);
}
...
sanity_check(void)
{
    ...
    for (this = base; this; prev = this, this = this->next)
    {
        ...
        if (this->next)
        {
            my_assert(
                (char *)this->next
                    == ((char *)this) + (this->size) + sizeof(struct chunk),
                4);
